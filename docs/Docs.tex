...
\begin{enumerate}
    ...
    \item Crear y activar un entorno virtual de Python \texttt{uv venv}.
    \item Instalar las dependencias con \texttt{uv sync} (o el archivo equivalente).
    \item Crear un archivo \texttt{.env} con la API key de Google (\texttt{GOOGLE\_API\_KEY=...}) y el modelo (\texttt{GEMINI\_MODEL=gemini-2.5-flash}).
    \item Ejecutar el backend con \texttt{uv run main.py}.
\end{enumerate}
