# app/tools/ast_parser/ast_nodes.py
...
class ArrayAccess(Expr):
    array: Union["Var", "ArrayAccess"] = field(default_factory=Var)
    index: Expr = field(default_factory=Literal)
    def to_dict(self) -> Dict[str, Any]:
        arr = self.array.to_dict() if hasattr(self.array, "to_dict") else {"type": "Var", "name": str(self.array)}
...
class UnOp(Expr):
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {"type": "UnOp", "op": self.op, "operand": self.operand.to_dict()}
...
class Compare(Expr):
    ...
    right: Expr = field(default_factory=Literal)
    def to_dict(self) -> Dict[str, Any]:
        return {"type": "Compare", "op": self.op, "left": self.left.to_dict(), "right": self.right.to_dict()}
...
class Assign(Stmt):
    ...
    def to_dict(self) -> Dict[str, Any]:
        out = super().to_dict()
        out.update({"target": self.target.to_dict(), "value": self.value.to_dict()})
        return out
...
class While(Stmt):
    ...
    def to_dict(self) -> Dict[str, Any]:
        out = super().to_dict()
        out.update({"cond": self.cond.to_dict(), "body": self.body.to_dict()})
        return out

@dataclass
class For(Stmt):
    """Bucle con rango inclusivo [start, end]."""
    var: str = ""
